...
uint8_t maxColor[3] = {255, 255, 255};

// uint8_t colorOriginal[12][3] = {
//     {50, 0, 0},   //red ~~
//     {50, 25, 0},  //orange ~~
//     {50, 50, 0},  //amarelo ~~
//     {50, 38, 0},  //castanho ~~
...
{
  uint8_t mod, tempPoint[3], invI, invJ, invX;
  uint16_t coords, tmp = 0;
...
void clearBack(void)
{
  for (uint8_t y = 0; y < NCOLS; y++)
  {
    for (uint8_t x = 0; x < NCOLS; x++)
    {
      setColor(backLayer[x + (y * NCOLS)], minColor);
    }
  }
}
...
void clearText(void)
{
  ...
  for (uint8_t y = 0; y < NCOLS; y++)
  {
    for (uint8_t x = 0; x < NCOLS; x++)
    {
      setColor(textLayer[x + (y * NCOLS)], minColor);
    }
  }
#endif
}
  ...
  for (uint8_t i = 0; i < NCHANNELS; i++)
  {
    for (uint8_t j = 0; j < NLINESCH; j++)
    {
      for (uint16_t x = 0; x < NCOLS; x++)
      {
        if ((x + (j * NCOLS) + (i * NLEDSCH)) > NLEDSL)
          break;
        WS2812_framedata_setPixel(i, x + (j * NCOLS), minColor[0], minColor[1], minColor[2]);
      }
    }
  }
}
