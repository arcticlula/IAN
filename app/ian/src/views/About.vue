				...
				<b-tab title="Settings" color="info">
					...
					<b-row>
						<b-col cols="12" class="mb-2">
							<b-button
								@click="sendCustom('WTECS')"
								style="width: 100%;"
								variant="info"
							>What The Eyes Cannot See</b-button>
						</b-col>
					</b-row>
					<b-row>
						<b-col cols="12" class="mb-2">
							<b-button-group style="width: 100%;">
								<b-button @click="sendCustom('EW1')" variant="info">
									Electric Waves
									<b-icon icon="circleHalf"></b-icon>
								</b-button>
								<b-button @click="sendCustom('EW2')" variant="danger">
									Electric Waves
									<b-icon icon="gridFill"></b-icon>
								</b-button>
							</b-button-group>
						</b-col>
					</b-row>
					<b-row>
						<b-col cols="12" class="mb-2">
							<b-button @click="sendCustom('BN')" variant="info" style="width: 100%;">Boarding now</b-button>
						</b-col>
					</b-row>
					<b-row>
						<b-col cols="12" class="mb-2">
							<b-button @click="sendCustom('BC')" variant="info" style="width: 100%;">Black Celebration</b-button>
						</b-col>
					</b-row>
					<b-row>
						<b-col cols="12" class="mb-2">
							<b-button-group style="width: 100%;">
								<b-button @click="sendCustom('NN1')" variant="info">
									No Name
									<b-icon icon="circleHalf"></b-icon>
								</b-button>
								<b-button @click="sendCustom('NN2')" variant="danger">
									No Name
									<b-icon icon="squareHalf"></b-icon>
								</b-button>
							</b-button-group>
					...
					<hr v-if="currentSong" />
					<b-row class="mt-4" v-if="currentSong">
						<b-col cols="6">
							<b-button-group>
								<b-button @click="sendCustomOr(0)" class="mr-1" variant="info">
									<b-icon-arrowsAngleExpand></b-icon-arrowsAngleExpand>
								</b-button>
								<b-button @click="sendCustomOr(1)" class="mr-1" variant="info">
									<b-icon-arrowsAngleContract></b-icon-arrowsAngleContract>
								</b-button>
							</b-button-group>
						</b-col>
						<b-col cols="6" style="text-align: right;">
							<b-button-group>
								<b-button @click="sendCustomDIV(-1)" variant="info">
									<b-icon-dash></b-icon-dash>
								</b-button>
								<b-button variant="outline-info" class="px-3" disabled>{{maxDiv}}</b-button>
								<b-button @click="sendCustomDIV(1)" class="mr-1" variant="info">
...
export default {
	...
	methods: {
		...
		sendCustom(song) {
			...
			let msg = this.customPatterns[song];
			this.maxDiv = this.customPatterns[song][4];
			this.settings.brightness = this.customPatterns[song][1];
			this.hdlc.minihdlc_send_frame(msg, msg.length);
		},
		sendCustomOr(orientation) {
			let msg = JSON.parse(
				JSON.stringify(this.customPatterns[this.currentSong])
			);
			msg[5] = orientation;
			this.hdlc.minihdlc_send_frame(msg, msg.length);
		},
		sendCustomDIV(sum) {
			...
			this.maxDiv += sum;
			if (this.maxDiv < 2) this.maxDiv = 2;
			if (this.maxDiv > 9) this.maxDiv = 9;
			msg[4] = this.maxDiv;
			this.hdlc.minihdlc_send_frame(msg, msg.length);
		},
		setShape(shape) {
